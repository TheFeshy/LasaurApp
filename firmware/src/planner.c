...
void planner_line(double x, double y, double z, double feed_rate, uint8_t nominal_laser_intensity) {
  ...
  if (target[X_AXIS] < position[X_AXIS]) { block->direction_bits |= (1<<OUTBITS_X_DIR_BIT); }
  if (target[Y_AXIS] < position[Y_AXIS]) { block->direction_bits |= (1<<OUTBITS_Y_DIR_BIT); }
  #ifdef OUTBITS_Z_DIR_BIT
    if (target[Z_AXIS] < position[Z_AXIS]) { block->direction_bits |= (1<<OUTBITS_Z_DIR_BIT); }
  #endif
